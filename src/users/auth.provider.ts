...
export class AuthProvider {
  ...
  /**
   * Create new user
   * @Body registerDto data for creating new user
  ...
  /**
   * Log In user
   * @Body loginDto data for log in to user account
  ...
  /**
   *  Sending set password link to the client
   */
  public async setPasswordLink(email: string) {
    const user = await this.usersRepository.findOne({ where: { email } });
    if (!user)
      throw new BadRequestException('user with given email does not exist');
    ...
    const setPasswordLink = `${this.configService.get<string>('CLIENT_DOMAIN')}/reset-password/${user.id}/${result.resetPasswordToken}`;
    await this.mailService.sendsetPasswordTemplate(email, setPasswordLink);
    return {
      message:
        'Password reset link sent to your email, please check your inbox',
    };
   ...
   */
  public generateLink(userId: number, verificationToken: string | null) {
